...
class Constants {
  ...
  static String convertPrice(BuildContext context, String price) {
    ...
    double cUSD = currencyMapUSD.isNotEmpty
        ? double.parse(currencyMapUSD['price']['c'].toString())
        : 0.0;
  ...
  static bool checkIsNewItem(String dateString) {
    ...
    final date2 = DateTime.now();
    final difference = date2.difference(date);
    if (difference.inDays == 0 || difference.inDays == 1) return true;
...
class Capture {
  Future<Uint8List> capturePng(
      BuildContext context, GlobalKey _globalKey) async {
    try {
